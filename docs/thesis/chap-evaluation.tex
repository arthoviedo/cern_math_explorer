...
Each of the users evaluated the results only for the equations he/she provided. For each of the presented result, the user ranked it as "Completely Relevant", "Partially Relevant" and "Completely Irrelevant". As all evaluations including human interaction, this scoring is at some level subjective. 

We present results for three metrics: Partial precision (Document either partially relevant or completely relevant) $P_{partial} = \frac{|Partially Relevant Results \cup Strictly Relevant Results | }{K}$, $P_{strict} = \frac{Strictly Relevant Results | }{K}$ and discounted cumulative gain.
